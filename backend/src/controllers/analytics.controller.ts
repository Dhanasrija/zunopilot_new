...
export const ordersByDay = asyncHandler(async (req, res) => {
  const tenantId = tenantIdOf(req);
  const sinceParam = queryString(req.query.since);
...
export const messageStats = asyncHandler(async (req, res) => {
  ...
  const base = { tenantId, direction: 'OUTBOUND', createdAt: { gte: since } } satisfies Prisma.MessageWhereInput;
  const prevBase = { tenantId, direction: 'OUTBOUND', createdAt: { gte: prevSince, lt: since } } satisfies Prisma.MessageWhereInput;
