...
export const upsertTemplate = asyncHandler(async (req, res) => {
  ...
  const template = await prisma.messageTemplate.upsert({
    where: { tenantId_trigger: { tenantId: req.tenantId, trigger } },
    update: { name, metaTemplate, language, body, isActive },
    create: { tenantId: req.tenantId, name, trigger, metaTemplate, language, body, isActive },
  });
...
export const deleteTemplate = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const t = await prisma.messageTemplate.findFirst({ where: { id, tenantId: req.tenantId } });
